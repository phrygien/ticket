...
new class extends Component {
    ...
    public function fetchTicketsByStatus($status, $append = false)
    {
        ...
        $url = env('API_REST') ."/ticket?page={$page}&status=" . urlencode($status) . "&label_id=11";
...
<x-header title="Inversion de colis" subtitle="Ticket demande d'inversion de colis" separator>
                    ...
                    >
                        ...
                        <div class="space-y-1.5 mb-3">
                            <div class="flex items-center gap-2 text-xs text-gray-600">
                                ...
                                <span class="font-medium">Date: {{ \Carbon\Carbon::parse(preg_replace('/\s*\([^)]+\)\s*$/', '', $ticket['created_at']))->setTimezone('Europe/Paris')->format('d/m/Y H:i') : '' }}</span>
                            </div>
                            <div class="flex items-center gap-2 text-xs text-gray-600">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1" class="size-4">
