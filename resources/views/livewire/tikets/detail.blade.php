...
new class extends Component {
    ...
    #[Validate(["photos.*" => "image|max:5120"])] // 5 Mo max
    public $photos = [];
    ...
    function formatMessage($message)
    {
        ...
        foreach ($links as $index => $link) {
            ...
            $href = preg_match("~^https?://~i", $link["url"])
                ? $link["url"]
                : "http://{$link["url"]}";
            $replacement =
                '<a href="' .
        ...
        $pattern =
            '~(?<!href=["\'])(?<!>)(https?://[^\s<]+|www\.[^\s<]+)(?![^<]*</a>)~i';
        $message = preg_replace_callback(
            $pattern,
            function ($matches) {
                $url = $matches[0];
                $href = preg_match("~^https?://~i", $url)
                    ? $url
                    : "http://$url";
                return '<a href="' .
                    e($href) .
    ...
    public function translateOpenAI()
    {
        ...
        if ($translateResponse->successful()) {
            ...
            if (!empty($translated)) {
                ...
                $this->success(
                    "Message traduit de {$currentLang} vers {$targetLang}",
                );
            } else {
            ...
            $this->error(
                "Erreur lors de la traduction : " . $translateResponse->body(),
            );
        }
    }
    ...
    public function translateMessage()
    {
        ...
        if ($response->successful()) {
            $translated =
                $response->json("translated_text") ?? "Erreur de traduction";
        ...
        if (!empty($this->photos)) {
            foreach ($this->photos as $file) {
                ...
                if (empty($fileContent)) {
                    $this->error(
                        "Le fichier {$file->getClientOriginalName()} est vide",
                    );
                    continue;
                }
                ...
                if (empty($base64Content)) {
                    $this->error(
                        "Erreur d'encodage pour {$file->getClientOriginalName()}",
                    );
                    continue;
                }
    ...
    public function downloadAttachment($messageIndex, $attachmentIndex)
    {
        ...
        if (
            ...
            str_starts_with($attachment["data"], "data:")
        ) {
            preg_match(
                "/data:([^;]+);base64,(.+)/",
                $attachment["data"],
    ...
    public function confirmerActionLu()
    {
        ...
        if ($response->successful()) {
            ...
            $this->error(
                "Erreur lors de la mise à jour : " . $response->body(),
            );
        }
    }
...
<div class="w-full mx-auto">
    ...
    <div class="mx-auto w-full mt-5">
        {{-- Tickets groupés (alignés avec les onglets) --}}

        <livewire:tikets.group-ticket :ticket-id="$ticketId" :active-id="$ticketId" />
        ...
        <div class="border-b border-gray-200 mb-5 mt-5">
            ...
            {{ $activeTab === 'description' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-200' }}">
                ...
                @if($chatbotMessageTab)
                    <button wire:click="setTab('chabotmessage')"
                            class="px-3 py-2 text-sm font-medium whitespace-nowrap border-b-2 relative
                {{ $activeTab === 'chabotmessage' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-200' }}">
                        <svg wire:loading.remove wire:target="setTab('chabotmessage')" class="w-4 h-4 inline-block mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h.01M15 12h.01M2 10v4M22 10v4" />
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"   d="M9 16c.667.667 1.667 1 3 1s2.333-.333 3-1" />
                        </svg>
                        <svg wire:loading wire:target="setTab('chabotmessage')" class="animate-spin w-4 h-4 inline-block mr-1" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
        ...
        <div class="relative">
            ...
            <div>
                ...
                @elseif($activeTab === 'chabotmessage')
                    <!-- Contenu chabotmessage -->
                    ...
                    @forelse(collect($this->messagesChatBot)->sortBy('date_created') as $msg)
                        ...
                        <div style="display: flex; justify-content: {{ $isClient ? 'flex-start' : 'flex-end' }};">
                            <div style="max-width: 75%; display: flex; flex-direction: column; align-items: {{ $isClient ? 'flex-start' : 'flex-end' }};">
                                ...
                                ">
                                    <p>{{!! $this->formatMessage(trim($msg['message'])) !!}}</p>
                                </div>
                            </div>
                        </div>
                    @empty
